...
import { FileText, Printer, Cloud, LogIn, LogOut, Loader2, Save, Download, Upload } from 'lucide-react';
...
const initialData: DocumentData = {
  ...
  quotationNotes: '1. กำหนดชำระค่าบริการก่อนวันที่ 30 ของทุกเดือน\n2. ค่าบริการข้างต้นยังไม่รวมปิดงบและตรวจสอบบัญชีประจำปี',
  invoiceNotes: '',
  receiptNotes: '',
  additionalDetails: '',
  discount: 0,
...
export default function App() {
  ...
  // Keyboard shortcut Ctrl+S or Cmd+S for quick save
  useEffect(() => {
  ...
  const executePdfGeneration = async (filename: string) => {
    ...
    try {
      ...
      const opt: any = {
        ...
        html2canvas:  { 
          ...
          onclone: (clonedDoc: any) => {
            ...
            // Ensure Google Thai Fonts are available in cloned head
            const fontLink = clonedDoc.createElement('link');
            fontLink.rel = 'stylesheet';
            fontLink.href = 'https://fonts.googleapis.com/css2?family=Kanit:wght@300;400;500;600;700&family=Prompt:wght@300;400;500;600;700&family=Sarabun:wght@300;400;500;600;700&display=swap';
      ...
      showToast("เกิดข้อผิดพลาดในการสร้าง PDF ระบบเปิดหน้าพิมพ์ให้อัตโนมัติ");
  ...
  const exportJsonBackup = () => {
    const docTypeName = data.type === 'QUOTATION' ? 'ใบเสนอราคา' : data.type === 'INVOICE' ? 'ใบแจ้งหนี้' : 'ใบเสร็จรับเงิน';
    const filename = `${docTypeName}_${data.documentNumber || 'ร่างเอกสาร'}.json`;
    ...
    showToast('ส่งออกไฟล์สำรอง JSON เรียบร้อย');
  };
  ...
  const importJsonBackup = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (importedData && importedData.type && importedData.items) {
          setData(importedData);
          setCurrentDocId(undefined);
          showToast('นำเข้าข้อมูลเอกสารเรียบร้อย');
        } else {
          alert('รูปแบบไฟล์ JSON ไม่ถูกต้องสำหรับเอกสารนี้');
        }
      } catch (err) {
  ...
  const executeSave = async (saveAsNew = false) => {
    ...
    try {
      ...
      showToast('เกิดข้อผิดพลาดในการบันทึกเอกสาร');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const docTypeName = data.type === 'QUOTATION' ? 'ใบเสนอราคา' : data.type === 'INVOICE' ? 'ใบแจ้งหนี้' : 'ใบเสร็จรับเงิน';
  const defaultPdfFilename = `${docTypeName}_${data.documentNumber || 'เอกสาร'}${data.to?.name ? '_' + data.to.name.replace(/[\s\\/:*?"<>|]/g, '_') : ''}`;
  const currentDocSummary = `${docTypeName} ${data.documentNumber ? 'เลขที่ ' + data.documentNumber : ''}${data.to?.name ? ' (' + data.to.name + ')' : ''}`;
    ...
    <div className="flex flex-col min-h-screen xl:h-screen xl:overflow-hidden bg-sand-100 text-stone-800 font-sans print:bg-white print:block print:h-auto print:min-h-0">
      ...
      <header className="bg-sand-50 border-b border-sand-200 px-4 md:px-6 py-3 flex flex-col md:flex-row md:items-center justify-between z-[100] no-print gap-4 shadow-sm shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {currentDocId && (
                <span className="hidden lg:inline-flex items-center gap-1 text-[11px] bg-amber-50 text-amber-800 border border-amber-200 px-2.5 py-0.5 rounded-full font-medium">
                  📄 บันทึกแล้ว ({data.documentNumber || 'เอกสาร'})
                </span>
              )}
            </div>
            {user ? (
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <button onClick={logOut} className="text-[10px] text-clay-500 hover:text-clay-700 underline cursor-pointer">ออกจากระบบ</button>
              </div>
            ) : (
        ...
        <div className="flex flex-wrap items-center gap-2 md:gap-3">
          <button
            onClick={resetForm}
            className="px-3.5 py-2 text-sm font-medium text-stone-600 bg-white border border-stone-200 hover:bg-sand-50 hover:text-stone-800 rounded-xl transition-colors focus:outline-none focus:ring-2 focus:ring-stone-200 shadow-sm cursor-pointer"
          >
            สร้างใหม่
          </button>
          ...
          {user ? (
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-stone-600 bg-white border border-stone-200 hover:bg-sand-50 hover:text-stone-800 rounded-xl transition-all duration-200 active:scale-95 focus:outline-none focus:ring-2 focus:ring-stone-200 shadow-sm cursor-pointer"
            >
              คลังเอกสาร
            </button>
          ) : (
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-stone-600 bg-white border border-stone-200 hover:bg-sand-50 hover:text-stone-800 rounded-xl transition-all duration-200 active:scale-95 focus:outline-none focus:ring-2 focus:ring-stone-200 shadow-sm cursor-pointer"
            >
          ...
          <button
            ...
            disabled={isSaving}
            title="กด Ctrl+S หรือ Cmd+S เพื่อบันทึก"
      ...
      <main className="flex-1 min-h-0 flex flex-col xl:flex-row w-full mx-auto relative xl:overflow-hidden print:overflow-visible print:block">
        ...
        <section className={`flex-1 p-0 sm:p-6 md:p-8 xl:p-12 xl:overflow-y-auto print:h-auto print:overflow-visible justify-center bg-sand-100 print:bg-white print:p-0 print:block custom-scrollbar items-start relative z-0 ${showMobilePreview ? 'flex' : 'hidden xl:flex'} print:!flex`}>
          <div className="w-full overflow-x-auto overflow-y-hidden custom-scrollbar pb-6 xl:pb-0 px-4 sm:px-0">
            <div className="bg-white shadow-xl shadow-stone-200 rounded-2xl print:shadow-none print:rounded-none w-[210mm] h-[297mm] max-h-[297mm] overflow-hidden mx-auto border border-sand-200 print:border-none print:w-full print:h-[297mm] relative">
              {/* Aspect ratio A4 for realistic preview */}
              <div id="document-preview-container" className="print-area print:h-[297mm] bg-white w-full h-[297mm] max-h-[297mm] flex flex-col justify-between print:static print:w-full overflow-hidden rounded-2xl print:rounded-none box-border">
      ...
      <div className="fixed bottom-6 right-6 z-[100] xl:hidden no-print">
        <button
          ...
          className="flex items-center gap-2 bg-leaf-600 hover:bg-leaf-700 text-white shadow-lg shadow-leaf-600/30 px-5 py-3 rounded-full font-medium transition-all transform active:scale-95 cursor-pointer"
        >
          {showMobilePreview ? (
            <>
      ...
      <PdfDownloadModal 
        ...
        onPrintNative={() => handlePrint(true)}
      ...
      <SaveDocumentModal
        ...
        documentInfo={currentDocSummary}
      />
