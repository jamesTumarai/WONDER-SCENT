              ...
              <div className="space-y-1.5 text-sm text-stone-600">
                <div className="flex flex-col items-start gap-3 mb-4">
                  {data.from.logo ? (
                    ...
                    <div className="relative shrink-0">
                      <img src="/33.png" alt="Company Logo" className="h-24 sm:h-28 object-contain object-left rounded-2xl" />
                    </div>
                  ) : null}
                  {!(data.from.logo || data.from.name.replace(/\s/g, '').toLowerCase().includes('วอนเดอร์เซ้นท์') || data.from.name.replace(/\s/g, '').toLowerCase().includes('wonderscent')) && <h1 className="text-2xl font-bold text-stone-800 tracking-tight">{data.from.name}</h1>}
                </div>
                {(data.from.logo || data.from.name.replace(/\s/g, '').toLowerCase().includes('วอนเดอร์เซ้นท์') || data.from.name.replace(/\s/g, '').toLowerCase().includes('wonderscent')) && (
                  <div className="pb-2">
                    <h1 className="text-xl font-bold text-stone-800 tracking-tight">{data.from.name}</h1>
                    {!data.from.name.toLowerCase().includes('wonder scent') && (
                      <h2 className="text-lg font-bold text-stone-600 tracking-tight uppercase mt-0.5">WONDER SCENT</h2>
                    )}
                  </div>
                )}
