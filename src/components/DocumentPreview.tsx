...
export default function DocumentPreview({ data }: Props) {
  ...
  const formatDate = (dateStr: string) => {
    ...
    if (isNaN(dayNum) || isNaN(yearNum)) return dateStr;
    return `${dayNum} ${getMonthName(m)} ${yearNum + 543}`;
  };
  ...
  const formatCurrency = (amount: any) => {
    ...
    return new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(safeNum);
  };

  const getBahtText = (amount: number) => {
    const safeVal = Math.max(0, Number(amount) || 0);
    try {
      return THBText(safeVal) || 'ศูนย์บาทถ้วน';
    } catch (e) {
      return 'ศูนย์บาทถ้วน';
    }
  };
  ...
  const subTotal = (data.items || []).reduce((sum, item) => {
    ...
    const p1 = Number(item.unitPrice) || 0;
    const p2 = data.columnSettings?.showPrice2 ? Number(item.unitPrice2) || 0 : 0;
    return sum + (q * p1) + (q * p2);
  }, 0);
  const afterDiscount = Math.max(0, subTotal - (Number(data.discount) || 0));
  const taxAmount = data.includeTax ? (afterDiscount * (Number(data.taxRate) || 0)) / 100 : 0;
  const grandTotal = afterDiscount + taxAmount;

  const theme = data.themeColor || 'stone';
  const colorMap = {
    stone: { bg: 'bg-[#703030]', text: 'text-[#703030]', border: 'border-[#703030]', label: 'text-stone-600', value: 'text-stone-800', receiptBg: 'bg-[#2A939C]', secBg: 'bg-[#DEEAEB]' },
    teal:  { bg: 'bg-[#2A939C]', text: 'text-[#2A939C]', border: 'border-[#2A939C]', label: 'text-[#1c5d64]', value: 'text-stone-900', receiptBg: 'bg-[#2A939C]', secBg: 'bg-[#DEEAEB]' },
  ...
  if (isQuotation) {
    return (
      <div className={`text-[12px] leading-[1.35] text-stone-800 bg-white h-[297mm] max-h-[297mm] box-border overflow-hidden relative flex flex-col justify-between font-medium`} style={{ fontFamily: fontStyle }}>
        <div className="pt-10 pb-10 px-12 flex flex-col flex-1 overflow-hidden">
                ...
                <tr>
                   <td colSpan={2} rowSpan={2 + (data.discount > 0 ? 1 : 0) + (data.includeTax ? 1 : 0)} className="px-4 align-middle bg-white">
                      <div className="flex items-center gap-8">
                         ...
                         <span className="flex-1 text-center font-normal">({getBahtText(grandTotal)})</span>
                      </div>
                   </td>
  ...
  return (
    <div className={`text-[12px] leading-[1.35] text-stone-800 bg-white h-[297mm] max-h-[297mm] box-border overflow-hidden relative flex flex-col justify-between font-medium`} style={{ fontFamily: fontStyle }}>
      <div className="pt-8 pb-8 px-12 flex flex-col flex-1 overflow-hidden">
        ...
        <div className="mt-auto print:mt-0 print:break-inside-avoid">
          ...
          <div className="flex justify-between items-start mb-6">
            ...
            <div className="w-1/2 flex flex-col items-end gap-2 text-[11px]">
               <div className="flex justify-between w-64 px-6">
                 <span className={labelColor}>{data.columnSettings?.amountLabel || 'ราคารวม'}</span>
          ...
          <div className="flex justify-between items-start mb-2">
            ...
            <div className="text-right">
              ...
              <div className={`text-[11px] ${labelColor} leading-none`}>( {getBahtText(grandTotal)} )</div>
            </div>
          </div>
