  ...
  return (
    <div className="relative w-full group">
      <Component 
        ...
        title={props.title || "ดับเบิ้ลคลิก หรือกดปุ่มขยายเพื่อพิมพ์เต็มหน้าจอ"}
      />
      <button
        type="button"
        onClick={() => setIsExpanded(true)}
        className="absolute right-2.5 top-2.5 p-1 text-stone-400 hover:text-stone-700 hover:bg-stone-100 rounded-lg transition-all opacity-40 group-hover:opacity-100 cursor-pointer"
          ...
          <div className="w-full max-w-2xl bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-4 border-b border-stone-100">
              ...
              <button
                ...
                className="p-2 text-stone-400 hover:text-stone-600 hover:bg-stone-100 rounded-xl transition-all duration-200 active:scale-90 cursor-pointer"
                type="button"
              >
                <X size={20} />
              </button>
            ...
            <div className="p-4 bg-stone-50 border-t border-stone-100 flex justify-end">
              <button
                onClick={() => setIsExpanded(false)}
                type="button"
                className="px-6 py-2.5 bg-stone-800 text-white font-medium rounded-xl hover:bg-stone-900 active:bg-black transition-all duration-200 active:scale-95 shadow-sm cursor-pointer"
              >
                เสร็จสิ้น
              </button>
            </div>
