...
export default function SaveDocumentModal({ isOpen, onClose, onSaveAsNew, onOverwrite, documentInfo }: SaveDocumentModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-[#1c191766] backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 border border-stone-200">
        ...
        {documentInfo && (
          <div className="mb-3 p-3 bg-sand-50 border border-sand-200 rounded-xl text-xs text-stone-700 font-medium">
            📄 เอกสารปัจจุบัน: <span className="font-bold text-stone-900">{documentInfo}</span>
          </div>
        )}
        ...
        <div className="flex flex-col gap-3">
          <button
            ...
            className="flex items-center justify-center gap-2 w-full py-3 px-4 text-leaf-700 bg-leaf-50 hover:bg-leaf-100 active:bg-leaf-200 border border-leaf-200 font-medium rounded-xl transition-all duration-200 active:scale-[0.98] shadow-sm cursor-pointer"
          >
            <Save size={18} />
            บันทึกแทนที่อันเดิม
          </button>
